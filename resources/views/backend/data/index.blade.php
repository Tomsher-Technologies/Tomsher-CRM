    ...
    <div class="card-body">
        ...
        <form method="GET" class="row g-3 mb-4">
            ...
            <div class="col-sm-3">
                <select name="source" class="form-control form-control-sm aiz-selectpicker" data-live-search="true">
                    ...
                    <option value="{{ $sour->id }}" {{ request('source')==$sour->id ? 'selected' : '' }}>{{ $sour->name
                        }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="row" style="text-align: -webkit-right;">
            ...
            <div class="col-md-2 m-auto">
                <form action="{{ route('data.index') }}" method="GET">
                    ...
                    <select name="sort_by" id="sort_by" class="form-control form-control-sm " data-live-search="true">
                        <option value="">Sort By</option>
                        <option value="next_followup_asc" {{ request('sort_by')=="next_followup_asc" ? 'selected' : ''
                            }}>
                            Next Follow-up ASC
                        </option>
                        <option value="next_followup_desc" {{ request('sort_by')=="next_followup_desc" ? 'selected' : ''
                            }}>
                            Next Follow-up DESC
                        </option>
                        <option value="last_updated_asc" {{ request('sort_by')=="last_updated_asc" ? 'selected' : '' }}>
                            Last Updated ASC
                        </option>
                        <option value="last_updated_desc" {{ request('sort_by')=="last_updated_desc" ? 'selected' : ''
                            }}>
                            Last Updated DESC
                        </option>
                    </select>
        ...
        <table class="table table-bordered aiz-table mb-0">
            ...
            <tbody>
                ...
                if(($dat->status == 'to_be_contacted' || $dat->status == 'contacted' || $dat->status == 'follow_up' ||
                $dat->status == 'not_responding') && ($dat->next_followup != NULL)){
                if($dat->next_followup === date('Y-m-d')){
                $bgcolor = '#e9efff';
                }

                if($dat->next_followup < date('Y-m-d')){ $bgcolor='#ffe0de' ; } } @endphp <tr
                    style="background:{{ $bgcolor }};">
                    <td class="text-center">{{ ($key+1) + ($data->currentPage() - 1)*$data->perPage() }}</td>
                    <td>{{ $dat->data_code }}</td>
                    <td style="word-break: break-word;">{{ $dat->company_name }}</td>
                    <td style="position: relative;word-break: break-word;">
                        ...
                        <div class="popup-card" id="popup-{{ $dat->id }}">
                            ...
                            <div class="popup-card-body">
                                ...
                                <div><i class="las la-mobile"></i> <strong>Mobile:</strong> {{ $primary->mobile_number
                                    }}</div>
                                @endif
                    ...
                    <td class="text-center">
                        {{ ucwords(str_replace('_',' ', $dat->status)) }}
                    ...
                    <td class="text-center">
                        ...
                        @if($dat->status != 'convert_to_enquiry')
                            <a href="javascript:void(0)"
                                ...
                                data-id="{{ $dat->id }}" data-status="{{ $dat->status }}" title="Change Status"
                                data-status-date="{{ $dat->last_updated }}" data-followup-date="{{ $dat->next_followup }}"
                                data-comment="{{ $dat->last_comment ?? '' }}">
                                <i class="las la-exchange-alt" style="margin-top: 2px;"></i>
                            </a>
                        @endif

                        <a href="javascript:void(0)" class="btn btn-secondary btn-sm btn-icon btn-circle view-timeline m-auto" data-id="{{ $dat->id }}">
...
<div class="modal fade" id="timelineModal" tabindex="-1" role="dialog" aria-labelledby="timelineModalLabel"
    ...
    $(document).on('click', '.view-timeline', function (e) {
        ...
        $('#timelineModalContent').html('<div class="text-center"><span class="spinner-border text-primary" role="status"></span></div>');
        ...
        $.ajax({
            ...
            error: function(){
                $('#timelineModalContent').html('<p class="text-danger">Failed to load timeline.</p>');
            }
        });
    });
