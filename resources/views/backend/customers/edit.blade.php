@extends('backend.layouts.app',['title' => 'Edit Customer'])

@section('content')

<div class="container">
    <div class="bg-white shadow-lg rounded-xl p-4">
        <h4 class="text-center mb-4">✏️ Edit Customer</h4>
        ...
        <form action="{{ route('customers.update', $customer->id) }}" method="POST" id="customerForm">
            ...
            <div class="mb-4">
                ...
                <div class="row">
                    <div class="col-md-4 mb-1">
                        ...
                        <input type="text" class="form-control form-control-sm" name="customer_code" value="{{ $customer->customer_code }}" readonly>
                    </div>
                    <div class="col-md-4 mb-1">
                    ...
                    <div class="col-md-4 mb-1">
                        ...
                        <input type="email" class="form-control form-control-sm" name="company_email" value="{{ $customer->company_email }}">
                    </div>
                    <div class="col-md-4 mb-1">
                        ...
                        <select class="form-control form-control-sm aiz-selectpicker" data-live-search="true" name="industry">
                            ...
                            @foreach ($industries as $ind)
                                <option value="{{ $ind->id }}" {{ $customer->industry_id == $ind->id ? 'selected' : '' }}>{{ $ind->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-1">
                        <label class="form-label">Website</label>
                        <input type="url" class="form-control form-control-sm" name="website" value="{{ $customer->website_link }}">
                    </div>
                    <div class="col-md-4 mb-1">
                        ...
                        <select class="form-control form-control-sm aiz-selectpicker" id="registered_country" name="registered_country"  data-live-search="true">
                            ...
                            @foreach ($countries as $con)
                                <option value="{{ $con->id }}" {{ ($con->id == $customer->company_country) ? 'selected' : '' }}>{{ $con->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-1">
                        ...
                        <select class="form-control form-control-sm aiz-selectpicker" id="emirate" name="emirate">
                            ...
                            @foreach ($emirates as $em)
                                <option value="{{ $em->id }}" {{ ($em->id == $customer->emirate) ? 'selected' : '' }}>{{ $em->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-1">
                        <label class="form-label">Company Address</label>
                        <textarea class="form-control form-control-sm" name="address" rows="3">{{ $customer->company_address }}</textarea>
                    </div>
                        ...
                        <select name="user_id" id="user_id" class="form-control form-control-sm aiz-selectpicker" data-live-search="true">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ old('user_id', $customer->sales_person ?? '') == $user->id ? 'selected' : '' }}>
                                    {{ $user->name  }} 
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 d-flex align-items-center" style="height: 100px;">
                        <div class="form-check mt-3">
                            <input class="form-check-input custom-checkbox" type="checkbox" name="new_to_company" value="1" {{ $customer->ntc ? 'checked' : '' }}>
                            <label class="form-check-label">New To Company (NTC)</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4 text-start">
                ...
                <a href="{{ Session::has('customers_last_url') ? Session::get('customers_last_url') : route('customers.index') }}" class="btn btn-info btn-sm">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
<script>
    let contactIndex = {{ $customer->contacts->count() }};
